...
function setBudget(event) {
    ...
    budgetAlert.textContent = `Budget set to ₹${budgetAmount.toFixed(2)} (${budgetFreq})`;
    checkBudget();
    updateBarChart();
}
...
function checkBudget() {
    ...
    if (budgetAmount > 0) {
        ...
        if (totalAmount > budgetAmount && !budgetExceededNotified) {
            ...
            if (budgetAlert) {
                budgetAlert.textContent = `Budget: ₹${budgetAmount.toFixed(2)} | Remaining: ${remainingPercentage.toFixed(2)}% (₹${(budgetAmount - totalAmount).toFixed(2)})`;
            }
        }
    }
}
...
function updateBarChart() {
    ...
    // Generate dynamic colors based on the frequency
...
function generateColors(count) {
    ...
    for (let i = 0; i < count; i++) {
        const color = `hsl(${(i * 360) / count}, 70%, 50%)`; // Generate unique colors
        colors.push(color);
    }
    return colors;
}
...
function initializePage() {
    ...
    if (currentPage === 'download-log.html') {
        displayDownloadLogs();
    }
...
// Function to initialize the book recommendation section
function initializeBookRecommendation() {
    const bookLinks = document.querySelectorAll('.dropdown-content a[data-image]');
    const bookImage = document.getElementById('book-image');
    const bookPdfLink = document.getElementById('book-pdf-link');

    if (bookLinks && bookImage && bookPdfLink) {
        bookLinks.forEach(link => {
            link.addEventListener('click', function (e) {
                e.preventDefault(); // Prevent default link behavior

                // Get the image and PDF file paths from the data attributes
                const imageSrc = link.getAttribute('data-image');
                const pdfSrc = link.getAttribute('data-pdf');

                // Set the book image source and display it
                bookImage.src = imageSrc;
                bookImage.style.display = 'block';

                // Set the PDF link href and display it
                bookPdfLink.href = pdfSrc;
                bookPdfLink.style.display = 'block';
            });
        });
    }
}

// Initialize the book recommendation section when the DOM is loaded
document.addEventListener('DOMContentLoaded', function () {
    initializeBookRecommendation();
});document.addEventListener('DOMContentLoaded', function() {
    const bookLinks = document.querySelectorAll('.dropdown-content a');
    const bookImage = document.getElementById('book-image');
    const bookPdfLink = document.getElementById('book-pdf-link');
...
if (protectedPages.includes(currentPage) && !localStorage.getItem("isLoggedIn")) {
    window.location.href = "pp.html"; // Redirect to login if not logged in
}
...
const API_KEY = 'uvufylvw';
const API_URL = 'mongodb+srv://root:040910@cluster--1.zn4po.mongodb.net/'; // Replace with your actual API endpoint
...
async function fetchExpenses() {
    try {
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                dataSource: 'Cluster--1', // cluster name
                database: 'dhanrakshak',
                collection: 'expenses',
        ...
        console.log('Fetched expenses:', data.documents);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    expenses.forEach(expense => {
        const li = document.createElement('li');
        li.textContent = `${expense.title} - ₹${expense.amount} (${expense.paymentMethod}) on ${expense.date} - Notes: ${expense.notes}`;
        expensesList.appendChild(li);
    });
});